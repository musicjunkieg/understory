...
Understory (`understory.watch`) is a social anti-algorithm for ATmosphereConf VODs built on AT Protocol. It crawls a user's Bluesky social graph, measures how much attention each conference talk got from their network, and surfaces the talks their timeline missed — combining inverted network attention, interest-based embedding similarity, and friend recommendations.

The full design spec lives in `docs/understory-design.md`.

## Architecture

### Three-Layer Scoring Engine (client-side)
1. **Network Attention (inverted)** — crawls follows' posts during the conference window (March 26–April 5, 2026), matches them to talks, then inverts: high attention = low score
2. **Interest Similarity** — cosine similarity between user's recent post embeddings and talk transcript embeddings
3. **Friend Overrides** — `watch.understory.recommendation` records published to follows' own PDS repos

Scoring runs entirely in the browser. No backend scoring server — all AT Protocol data is public.

### Custom Lexicon Schemas (`watch.understory.*`)
- `talkRef` — joins `place.stream.video` to `community.lexicon.calendar.event`
- `transcript` — timestamped, speaker-attributed transcript segments
- `topicIndex` — dense embeddings + topic labels for interest matching
- `recommendation` — user-published social recommendations (lives on each user's PDS)

### Offline Data Pipeline
Batch processing scripts in `scripts/`:
- `npm run transcribe` — extracts audio from HLS streams via ffmpeg, sends to AssemblyAI, saves to `data/transcripts/{rkey}.json`
- `npm run build-talk-index` — fetches VOD + schedule data from AT Protocol, outputs `data/talks.json`

### Frontend
- Next.js 16 with App Router, TypeScript, Tailwind CSS
- HLS.js for video playback
- Tailwind CSS with custom "bioluminescent understory" theme (dark forest palette, glow effects on undiscovered talks)
- AT Protocol OAuth for user authentication

## Design System

The visual metaphor is a forest floor at night. Undiscovered talks glow bioluminescent green; well-covered talks fade into the background. Full design system spec: `docs/superpowers/specs/2026-04-06-bioluminescent-design-system.md`. Visual reference: Stitch mockups in `docs/stitch/`. Fonts: Newsreader (display/headlines), Work Sans (body), Space Grotesk (labels/technical).

## Key External Dependencies

- **Bluesky/AT Protocol APIs**: `getFollows`, `getAuthorFeed`, `searchPosts`, `listRecords`, OAuth
- **Streamplace**: HLS VOD playback via `place.stream.playback.getVideoPlaylist`
- **Community Lexicon**: `community.lexicon.calendar.event` for conference schedule data
- **AssemblyAI**: speech-to-text (API key in `.env`, do not commit)

## Key Data Sources

- **VOD records**: `did:plc:rbvrr34edl5ddpuwcubjiost` on PDS `iameli.com` — `place.stream.video` collection
- **Schedule events**: `did:plc:3xewinw4wtimo2lqfy5fm5sw` (`atmosphereconf.org`) on `bsky.social` — `community.lexicon.calendar.event` collection (145 events, 96 with `vodAtUri` links)
- Full-day room streams are filtered by `creator === REPO_DID`; individual talk VODs have different creator DIDs

## Domain & Namespace

- Domain: `understory.watch`
